            ...
            for (let j = 0; j < fullPaths[i].length; j++) {
                if (fullPaths[i][j].length > 1) {
                    ...
                    for (let k = 0; k < fullPaths[i][j].length; k++) {
                        path += stackStore.stack[fullPaths[i][j][k]].key.substring(stackStore.stack[fullPaths[i][j][k]].key.length - 2) + '->'
                    }
                    path = path.substring(0, path.length - 2)
                    tablePaths.push({
                        key: String(Math.random()),
                        from: stackStore.stack[fullPaths[i][j][0]].key.substring(stackStore.stack[fullPaths[i][j][0]].key.length - 2),
                        to: stackStore.stack[fullPaths[i][j].at(-1)].key.substring(stackStore.stack[fullPaths[i][j].at(-1)].key.length - 2),
                        path: path,
